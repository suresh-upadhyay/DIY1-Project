...
func clearStoreTable() {
	a.DB.Exec("DELETE FROM store")
	a.DB.Exec("ALTER SEQUENCE store_s_id_seq RESTART WITH 1")
...
func addStore(count int){
	if count < 1 {
		count = 1
	}

	for i := 0; i < count; i++ {
		 a.DB.Exec("INSERT INTO store VALUES ($1, $2 , $3) ;", strconv.Itoa(i+1), strconv.Itoa((i+1.0)*10), true)
	}
}
...
func TestGetStoreProduct(t *testing.T) {
    clearStoreTable()
    addStore(1)
	req, _ := http.NewRequest("GET", "/store/1", nil)
	response := executeRequest(req)
